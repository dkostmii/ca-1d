...
int
cycle (int value, int min, int max)
{
  if (max < min)
    {
      int temp = min;
...
int
digits (int value, int radix, int **dest, int expected_length)
{
...
int
from_digits (int *source, int source_len, int radix)
{
  if (source == NULL)
    {
      return -1;
